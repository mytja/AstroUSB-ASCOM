// ASCOM Switch hardware class for mytjaAstroUSB
//
// Description:	 ASCOM driver for AstroUSB - a hacked version of Orico's 7-port USB hub
//
// Implements:	ASCOM Switch interface version: v2 (I think)
    ...
    internal static class SwitchHardware
    {
        ...
        static SwitchHardware()
        {
            try
            {
                ...
                tl = new TraceLogger("", "mytjaAstroUSB.Hardware");
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show($"{ex.Message}", "Exception creating ASCOM.mytjaAstroUSB.Switch", MessageBoxButtons.OK, MessageBoxIcon.Error);
                throw;
            }
        }
        ...
        private static short numSwitch = 7;
        ...
        internal static void SetSwitchValue(short id, double value)
        {
            ...
            throw new ASCOM.MethodNotImplementedException($"SetSwitchValue({id}) - Not supported");
        }
        ...
        internal static void ReadProfile()
        {
            using (Profile driverProfile = new Profile())
            {
                ...
                switches = LoadDefaultSwitches();
                //allSwitches = LoadDefaultSwitches();
                for (int i = 0; i < switches.Count; i++)
                {
                    /*if (!switches[i].CanSetName)
                    {
                        //switches.Add(switches[i]);
                        continue;
                    }*/
                    switches[i].Name = driverProfile.GetValue(DriverProgId, $"usb{switches[i].InternalID}_name", string.Empty, switches[i].Name);
                }
        ...
        internal static void WriteProfile()
        {
            using (Profile driverProfile = new Profile())
            {
                ...
                for (int i = 0; i < switches.Count; i++)
                {
                    ...
                    driverProfile.WriteValue(DriverProgId, $"usb{switches[i].InternalID}_name", switches[i].Name);
        ...
        public static List<LocalSwitch> LoadDefaultSwitches()
        {
            ...
            switches.Add(new LocalSwitch("USB1", "0") { Description = "USB1 switch" });
            switches.Add(new LocalSwitch("USB2", "1") { Description = "USB2 switch" });
            switches.Add(new LocalSwitch("USB3", "2") { Description = "USB3 switch" });
            switches.Add(new LocalSwitch("USB4", "3") { Description = "USB4 switch" });
            switches.Add(new LocalSwitch("USB5", "4") { Description = "USB5 switch" });
            switches.Add(new LocalSwitch("USB6", "5") { Description = "USB6 switch" });
            switches.Add(new LocalSwitch("USB7", "6") { Description = "USB7 switch" });
            return switches;
        }
